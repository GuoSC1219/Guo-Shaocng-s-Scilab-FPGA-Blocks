function [x, y, typ]= seu_dpd_opfb_4_3_32ch(job, arg1, arg2)
    x=[];y=[];typ=[];
    blkname = 'seu_dpd_opfb_4_3_32ch';
    select job
      case 'set' then
        x=arg1;
        ...
        [ok, blkname, exprs] = scicos_getvalue("Set seu_dpd_opfb_4_3_32ch parameters",...
                          txt,...
                          list("str", 1),...
                          exprs);
        if ok then
      ...
      case 'define' then
        model = scicos_model();
        model.sim = list('seu_dpd_opfb_4_3_32ch',4);
        model.blocktype = 'c';
        ...
        debug_info('seu_dpd_opfb_4_3_32ch loaded...')
    end
  endfunction
